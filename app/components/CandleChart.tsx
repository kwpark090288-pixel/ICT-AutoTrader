...
========================= */
const H4_STORE = new Map<string, H4Context>();
...
function buildChannelFromScratch(candles4h: Candle[]): ChannelState {
  ...
  if (isUp) {
    ...
    if (!upPair) return { mode: "none", breakCount: 0 };

    const base: Line2P = { t1: upPair.L1.t, p1: upPair.L1.p, t2: upPair.L2.t, p2: upPair.L2.p };

    // 2-a) residual 표본 제한: base.t2 이후 pivotHigh만
    const residuals: number[] = [];
    for (const ph of pivotHighs) {
      if (Number(ph.t) < Number(base.t2)) continue;
  ...
  if (!dnPair) return { mode: "none", breakCount: 0 };

  const base: Line2P = { t1: dnPair.H1.t, p1: dnPair.H1.p, t2: dnPair.H2.t, p2: dnPair.H2.p };

  // 2-a) residual 표본 제한: base.t2 이후 pivotLow만
  const residuals: number[] = [];
  for (const pl of pivotLows) {
    if (Number(pl.t) < Number(base.t2)) continue;
...
function updateLockedChannel(prev: ChannelState, candles4h: Candle[]): ChannelState {
  ...
  const atrNow = Number.isFinite(atr[i]) ? atr[i] : NaN;
  ...
  let valid = false;
  if (prev.mode === "up") {
    const breach = lineP - c.close;
    valid = c.close < lineP && breach >= atrNow * BREAK_ATR_MULT;
  } else if (prev.mode === "down") {
    const breach = c.close - lineP;
    valid = c.close > lineP && breach >= atrNow * BREAK_ATR_MULT;
  }

  const nextCount = valid ? prev.breakCount + 1 : 0;
  if (nextCount >= 2) {
    return buildChannelFromScratch(candles4h);
  }
  return { ...prev, breakCount: nextCount };
}
...
function buildTrendlineFromScratch(
  ...
  const atr = computeATR(candles4h, 14);
  const atrNow = Number.isFinite(atr.at(-1) ?? NaN) ? (atr.at(-1) as number) : NaN;
  if (!Number.isFinite(atrNow) || atrNow <= 0) return keepIfRange ?? { mode: "none", breakCount: 0 };
  ...
  if (!isUp && !isDown) {
    // mixed/range => 기존 locked 유지
    return keepIfRange ?? { mode: "none", breakCount: 0 };
  }

  if (isUp) {
    const pair = pickHLPair(pivotLows, atrNow, TREND_MIN_SWING_MULT);
    if (!pair) return keepIfRange ?? { mode: "none", breakCount: 0 };
...
function passesContextDistance(
  ...
  const distMax = atrNow * 0.25;

  // 채널 존재 시 채널 우선 + 방향 일치
  if (channel.mode !== "none" && channel.base && channel.offset) {
    if (channel.mode === "up") {
      if (side !== "bull") return false;
      const support = linePriceAt(channel.base, t);
      return Math.abs(midPrice - support) <= distMax;
    }
    if (channel.mode === "down") {
      if (side !== "bear") return false;
      const resist = linePriceAt(channel.base, t);
      return Math.abs(midPrice - resist) <= distMax;
    }
  }
  ...
  if (trend.mode !== "none" && trend.line) {
    if (trend.mode === "up") {
      if (side !== "bull") return false;
      const lp = linePriceAt(trend.line, t);
      return Math.abs(midPrice - lp) <= distMax;
    }
    if (trend.mode === "down") {
      if (side !== "bear") return false;
      const lp = linePriceAt(trend.line, t);
...
function computeAPlusOBAndAFVG(
  ...
  for (let k = 0; k < n; k++) {
    ...
    // 컨텍스트/거리 + 방향 일치 필수
    if (!passesContextDistance(side, mid, ob.time, atrK, channel, trend)) continue;
  ...
  for (let i = 1; i < n - 1; i++) {
    ...
    const mid = (top + bottom) / 2;
    const F4 = passesContextDistance(side, mid, cMid.time, atrI, channel, trend);
...
async function fetchBinanceKlinesPaginated(params: {
  ...
  return merged.length > targetCount ? merged.slice(merged.length - targetCount) : merged;
}
...
export default function CandleChart({ symbol, tf }: Props) {
  ...
  const drawOverlay = useCallback(() => {
    ...
    try {
      ...
      const forwardSec = MAX_FORWARD_BARS * tfToSeconds(tf);
